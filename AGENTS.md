...
- When the SQLite schema changes, update the canonical `CREATE TABLE` definitions and directly mutate `data/news.sqlite` during the same task. Preserve relevant local data and verify the resulting schema.
- When database-backed settings or their import/export shape changes, also
  update the ignored `newsbrew.json` and `newsbrew.dev.json` files when they
  exist. Preserve their file-specific values and secrets, write them with
  owner-only permissions, and validate them against the current configuration
  schema.
...
- Keep refresh writes atomic. Filter results, seen IDs, and analysed articles are staged and committed together; stopping or aborting a refresh must not leave a partial ingestion run.
- Store tests must set `NEWSBREW_CONFIG_JSON` to a temporary database before importing `store.ts`. Never let tests open `data/news.sqlite`, and check the live database for exact fixture contamination if isolation fails.
...
- Treat SQLite data, settings exports, tuning JSONL, benchmark configs,
  ...
  benchmark artifacts under ignored `data/benchmark-*` paths; never put a
  reader's identity, machine paths, credentials, preferences, or labeled
